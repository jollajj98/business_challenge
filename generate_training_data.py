...
for x in range(1000):
    risk_level.append(random.choice([0, 1, 2, 3]))
    if risk_level[x] == 0:
...
column_names=["risk_level","vaccinated","intensive_care","hospitalised","positive","reproduction","mortality"]
df = pd.DataFrame([risk_level,vaccinated,intensive_care,hospitalised,positive,reproduction,mortality]).T
df.columns = column_names
print(df)
print(df.dtypes)
df.to_csv("risk_level_stats.csv")